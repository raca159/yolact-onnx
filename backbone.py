...
def dcn_v2_conv(input, offset, mask, weight, bias,
        ...
        ctx_stride = _pair(stride)
        ctx_padding = _pair(padding)
        ctx_dilation = _pair(dilation)
        ctx_kernel_size = _pair(weight.shape[2:4])
        ctx_deformable_groups = deformable_groups
        return _backend.dcn_v2_forward(input, weight, bias,
                                         offset, mask,
                                         ctx_kernel_size[0], ctx_kernel_size[1],
                                         ctx_stride[0], ctx_stride[1],
